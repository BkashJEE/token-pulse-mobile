...
export function isLocalDashboardRequest(request, {
  environment = process.env.NODE_ENV,
  localNoAuth = process.env.TOKEN_PULSE_LOCAL_NO_AUTH,
} = {}) {
  if (environment !== 'development' && localNoAuth !== '1') return false;
  try {
    const url = new URL(request.url);
    return url.protocol === 'http:' && LOCAL_DASHBOARD_HOSTS.has(url.hostname);
  } catch {
    return false;
  }
}
